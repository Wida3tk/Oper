...
export async function GET(req:Request){
 ...
 if(cookieValue){const digest=await tokenHash(decodeURIComponent(cookieValue)),session=await db.prepare("SELECT s.email,a.display_name,a.permissions FROM staff_sessions s JOIN staff_accounts a ON a.email=s.email WHERE s.token_hash=? AND s.expires_at>? AND a.active=1").bind(digest,new Date().toISOString()).first<{email:string;display_name:string;permissions:string}>();if(session){const {results}=await db.prepare("SELECT role FROM staff_roles WHERE email=? AND active=1").bind(session.email).all<{role:string}>();let permissions:string[]=[];try{permissions=JSON.parse(session.permissions||"[]")}catch{}return Response.json({authenticated:true,email:session.email,name:session.display_name||session.email.split("@")[0],roles:bootstrapAdmins.has(session.email)?["admin"]:results.map(x=>x.role),permissions:bootstrapAdmins.has(session.email)?["*"]:permissions})}}
