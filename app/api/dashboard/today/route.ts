...
export async function GET(req: Request) {
  ...
  const [tasks, customers, payments, enrollments] = await Promise.all([
    db.prepare("SELECT id,title,department,priority,due_at,status FROM workflow_tasks WHERE status!='مكتملة' AND (assignee_email=? OR assignee_email IS NULL) ORDER BY CASE priority WHEN 'عاجلة' THEN 0 WHEN 'عالية' THEN 1 ELSE 2 END,due_at LIMIT 8").bind(auth.email).all(),
  ...
  return Response.json({ user: { email: auth.email, name, roles: auth.roles }, today, stats: { tasks: tasks.results.length, customers: Number(customers?.count||0), payments: Number(payments?.count||0), paymentAmount: Number(payments?.amount||0), enrollments: Number(enrollments?.count||0) }, tasks: tasks.results });
}
