...
export async function GET(req:Request){
 ...
 const orders=ordersResult.results.map(order=>{
  ...
  const withdrawal=withdrawalsResult.results.find(x=>x.order_id===order.order_id),grossPaidCents=orderPayments.reduce((sum,x)=>sum+cents(x.amount),0),refundCents=cents(withdrawal?.refund_amount),paidCents=Math.max(grossPaidCents-refundCents,0),totalCents=cents(order.stored_total),remainingCents=withdrawal?0:Math.max(totalCents-paidCents,0);
...
export async function POST(req:Request){
 ...
 const db=operationalDb(),now=new Date().toISOString(),order=await db.prepare("SELECT id,total,base_total,discount_percent,order_type,purchase_source,finance_review_status FROM orders WHERE id=?").bind(orderId).first<{id:string,total:number;base_total:number;discount_percent:number;order_type:string;purchase_source:string;finance_review_status:string}>();
 await ensureFinanceUndoSchema(db);
 await ensureWithdrawalSchema(db);
 await ensureOrderPaymentReferenceSchema(db);
 ...
 if(action==="register_withdrawal"){
  ...
  const existing=await db.prepare("SELECT id FROM withdrawals WHERE order_id=?").bind(orderId).first();if(existing)return Response.json({error:"تم تسجيل انسحاب لهذا العميل مسبقاً"},{status:409});
  const reason=String(body.reason||"").trim(),withdrawnAt=String(body.withdrawnAt||""),refundSource=String(body.refundSource||""),refundMethod=String(body.refundMethod||"").trim(),reference=String(body.reference||"").trim(),refundCents=cents(body.refundAmount),nonRefundableCents=cents(body.nonRefundableAmount);
  if(!reason||!/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(withdrawnAt)||!["sale","collection"].includes(refundSource)||!refundMethod||!reference)return Response.json({error:"سبب وتاريخ ومصدر وطريقة ومرجع الاسترداد مطلوبة"},{status:400});
  if(refundCents<0||nonRefundableCents<0||refundCents+nonRefundableCents>paidCents)return Response.json({error:"مجموع المسترد وغير المسترد لا يمكن أن يتجاوز إجمالي المدفوع"},{status:400});
  const withdrawalId=id("WDR"),refundAmount=money(refundCents),nonRefundableAmount=money(nonRefundableCents),grossPaid=money(paidCents),netPaid=money(paidCents-refundCents);
  await db.batch([
   db.prepare("INSERT INTO withdrawals(id,order_id,reason,withdrawn_at,gross_paid,non_refundable_amount,refund_amount,refund_source,refund_method,reference,status,created_by_email,created_at) VALUES(?,?,?,?,?,?,?,?,?,?, 'مكتمل',?,?)").bind(withdrawalId,orderId,reason,withdrawnAt,grossPaid,nonRefundableAmount,refundAmount,refundSource,refundMethod,reference,auth.email,now),
   db.prepare("UPDATE installments SET status='ملغي',updated_at=? WHERE order_id=? AND status!='مدفوع'").bind(now,orderId),
 ...
 const lockedWithdrawal=await db.prepare("SELECT id FROM withdrawals WHERE order_id=?").bind(orderId).first();
 if(lockedWithdrawal&&!['note','update_payment_reference'].includes(action))return Response.json({error:"الملف منسحب ومغلق أمام الإجراءات المالية الجديدة"},{status:409});
 if(action==="undo_last_finance_action"){
  ...
  }else if(target.action==="UPDATE_PAYMENT_RECORD_DATE"){
   statements.push(db.prepare("UPDATE payments SET paid_at=? WHERE id=? AND order_id=?").bind(details.previousPaidAt||null,target.entity_id,orderId),db.prepare("UPDATE installments SET paid_at=?,updated_at=? WHERE paid_payment_id=? AND order_id=?").bind(details.previousPaidAt||null,now,target.entity_id,orderId));
 ...
 if(action==="schedule"){
  ...
  const statements=[db.prepare("UPDATE orders SET total=?,paid=?,payment_plan='أقساط',status=?,updated_at=? WHERE id=?").bind(money(totalCents),money(paidCents),remaining===0?"مدفوع":paidCents>0?"مدفوع جزئياً":"غير مدفوع",now,orderId),db.prepare("DELETE FROM installments WHERE order_id=? AND status!='مدفوع'").bind(orderId)];
 ...
 if(action==="update_payment_record_date"){
  if(!can(auth,"finance.payments.record"))return Response.json({error:"ليس لديك صلاحية تعديل تاريخ السداد"},{status:403});
  const paymentId=String(body.paymentId||""),paymentDate=String(body.paymentDate||"");
  if(!/^\d{4}-\d{2}-\d{2}$/.test(paymentDate))return Response.json({error:"تاريخ السداد غير صالح"},{status:400});
  const payment=await db.prepare("SELECT id,paid_at FROM payments WHERE id=? AND order_id=?").bind(paymentId,orderId).first<{id:string;paid_at:string|null}>();
  if(!payment)return Response.json({error:"سجل الدفعة غير موجود"},{status:404});
  const paidAt=`${paymentDate}T12:00:00.000Z`;
  if(String(payment.paid_at||"").slice(0,10)===paymentDate)return Response.json({ok:true,unchanged:true});
  await db.batch([
   db.prepare("UPDATE payments SET paid_at=? WHERE id=? AND order_id=?").bind(paidAt,paymentId,orderId),
   db.prepare("UPDATE installments SET paid_at=?,updated_at=? WHERE paid_payment_id=? AND order_id=?").bind(paidAt,now,paymentId,orderId),
   db.prepare("INSERT INTO audit_log(id,actor_email,action,entity_type,entity_id,details,created_at) VALUES(?,?,'UPDATE_PAYMENT_RECORD_DATE','payment',?,?,?)").bind(id("AUD"),auth.email,paymentId,JSON.stringify({orderId,previousPaidAt:payment.paid_at,paymentDate:paidAt}),now)
  ]);
  return Response.json({ok:true,paymentDate:paidAt});
 }
