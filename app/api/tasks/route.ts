...
export async function GET(req: Request) {
  ...
  const query = mine
    ? db.prepare(`${details} WHERE t.assignee_email=? AND t.status!='مكتملة' AND ${visibility} ORDER BY CASE t.priority WHEN 'عاجلة' THEN 0 WHEN 'عالية' THEN 1 ELSE 2 END,t.due_at`).bind(auth.email)
    : db.prepare(`${details} WHERE t.status!='مكتملة' AND ${visibility} ORDER BY CASE t.priority WHEN 'عاجلة' THEN 0 WHEN 'عالية' THEN 1 ELSE 2 END,t.due_at LIMIT 200`);
