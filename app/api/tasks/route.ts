...
export async function GET(req: Request) {
  ...
  const personalScope = mine || !isAdmin;
  const query = personalScope
  ...
  const completedQuery = personalScope
    ? db.prepare(`${details} WHERE t.assignee_email=? AND t.status='مكتملة' AND date(t.completed_at)=date('now') AND ${visibility} ORDER BY t.completed_at DESC LIMIT 100`).bind(auth.email)
    : db.prepare(`${details} WHERE t.status='مكتملة' AND date(t.completed_at)=date('now') AND ${visibility} ORDER BY t.completed_at DESC LIMIT 100`);
  const [{ results },completed]=await Promise.all([query.all(),completedQuery.all()]);
