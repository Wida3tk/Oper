...
export async function GET(req: Request) {
  ...
  const { results } = await operationalDb().prepare(`
    SELECT c.id,c.name,c.phone,c.email,c.customer_type,c.admitted_via,c.created_at,
           ...
           CASE WHEN o.seat_reservation=1 THEN COALESCE((SELECT MAX(r.fee_amount) FROM seat_reservations r WHERE r.order_id=o.id AND r.reservation_kind='حجز مقعد'),0) ELSE 0 END seat_fee,
           p.name program_name
    FROM customers c
