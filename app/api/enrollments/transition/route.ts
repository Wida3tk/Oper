...
const transitions:Record<string,{from:string[],to:string,column?:string,next?:string}>={
  ...
  activated:{from:["اكتمل التسجيل","تم إنشاء الحساب","تم الإسناد"],to:"مكتمل",column:"completed_at"},
...
export async function POST(req:Request){
  ...
  if(transition.next)statements.splice(1,0,db.prepare("INSERT INTO workflow_tasks(id,entity_type,entity_id,department,title,status,priority,created_by_email,created_at) VALUES(?,'enrollment',?,'التشغيلية',?,'مفتوحة','عالية',?,?)").bind(id("TSK"),enrollmentId,transition.next,auth.email,now));
