...
function B2BWorkspace({section,canManage,canConvert}:{section:"business"|"partnerships";canManage:boolean;canConvert:boolean}) {
  const [rows,setRows]=useState<B2BRow[]>([]),[options,setOptions]=useState<string[]>([]),[loading,setLoading]=useState(true),[error,setError]=useState(""),[showForm,setShowForm]=useState(false),[selected,setSelected]=useState<B2BRow|null>(null),[saving,setSaving]=useState(false),[activities,setActivities]=useState<B2BRow[]>([]),[activityType,setActivityType]=useState("تم التواصل"),[activityDetails,setActivityDetails]=useState(""),[scope,setScope]=useState("assigned"),[search,setSearch]=useState(""),[stageFilter,setStageFilter]=useState("الكل"),[pathFilter,setPathFilter]=useState("الكل");
  ...
  const load=async()=>{setLoading(true);setError("");try{const data=await apiJson(`/api/b2b?section=${section}`);setRows(data.opportunities||data.partnerships||[]);setOptions(data.stages||data.statuses||[]);setScope(data.scope||"assigned")}catch(e){setError((e as Error).message)}finally{setLoading(false)}};
  ...
  return <div className="b2b-workspace">
    <section className="b2b-hero"><div><span>{scope==="all"?"نظرة شاملة لجميع الفرق":"مساحة العمل المسندة إليك"}</span><h2>{section==="business"?"مبيعات الأعمال":"الشراكات"}</h2><p>{section==="business"?"كل جهة، تواصل، اجتماع وعرض في مسار واحد واضح.":"متابعة الاتفاقيات والتفعيل والتجديد دون خلطها بمالية الأفراد."}</p></div>{section==="business"&&canManage&&<button onClick={()=>setShowForm(true)}><UserPlus size={17}/> إضافة جهة</button>}</section>
