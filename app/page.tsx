            ...
            <div className="b2b-drawer-body">
              <section className="b2b-account-profile">
                ...
                <div className="b2b-inline-contacts">
                  <div className="b2b-progress-head"><div><h3>ممثلو الجهة</h3><p>جهات التواصل المرتبطة بالبيانات الأولية</p></div><div className="b2b-contact-head-actions"><em>{contacts.length} ممثل</em>{canManage&&<button type="button" onClick={()=>setShowContactForm(value=>!value)}><Plus size={14}/>{showContactForm?"إغلاق":"إضافة ممثل إضافي"}</button>}</div></div>
                  <div className="b2b-contact-list">{contacts.map(contact=><article key={String(contact.id)}><div><b>{contact.name}</b><span>{contact.job_title||"دون مسمى"}</span><small dir="ltr">{[contact.phone,contact.email].filter(Boolean).join(" · ")||"دون بيانات تواصل"}</small></div>{Number(contact.is_primary)===1?<em>أساسي</em>:<button disabled={saving} onClick={()=>void execute({action:"save_contact",accountId:selected.account_id,contactId:contact.id,name:contact.name,jobTitle:contact.job_title||"",phone:contact.phone||"",email:contact.email||"",isPrimary:true})}>تعيين كأساسي</button>}<button className="remove" disabled={saving||contacts.length<=1} onClick={()=>void execute({action:"delete_contact",accountId:selected.account_id,contactId:contact.id})}>حذف</button></article>)}</div>
...
function B2BIntakeDecisionPanel({selected,approvals,canApprove,saving,execute}:{selected:B2BRow;approvals:B2BRow[];canApprove:boolean;saving:boolean;execute:(body:Record<string,unknown>)=>Promise<void>}){
  ...
  const label=status==="approved"?"مقبولة للتخطيط":status==="idea"?"فكرة أولية":"بانتظار قرار الفرز";
  return <section className="b2b-intake-decision"><div><span>قرار فرز الجهة</span><h3>{label}</h3><p>تحديد ما إذا كانت الجهة فرصة جاهزة للتخطيط والتواصل، أو فكرة تحفظ للمراجعة لاحقًا.</p></div>{canApprove&&<div><button className="accept" disabled={saving} onClick={()=>void execute({action:"record_approval",accountId:selected.account_id,opportunityId:selected.opportunity_id,approvalType:"مدير الشراكات",decision:"approved"})}><Check size={15}/> قبول للتخطيط</button><button className="idea" disabled={saving} onClick={()=>void execute({action:"record_approval",accountId:selected.account_id,opportunityId:selected.opportunity_id,approvalType:"مدير الشراكات",decision:"idea"})}>حفظ كفكرة أولية</button></div>}</section>
}
