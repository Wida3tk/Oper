...
function B2BApprovalsPanel({selected,approvals,canApprove,saving,execute}:{selected:B2BRow;approvals:B2BRow[];canApprove:boolean;saving:boolean;execute:(body:Record<string,unknown>)=>Promise<void>}){
  ...
  return <section className="b2b-approvals"><div className="b2b-progress-head"><div><h3>مسار الاعتماد</h3><p>تأكيدات مختصرة وموثقة دون تعطيل متابعة الجهة</p></div></div><div>{types.map(type=>{const row=latest(type),status=String(row?.status||"pending");return <article key={type}><i className={status}>{status==="approved"?<Check size={15}/>:status==="rejected"?<X size={15}/>:<Clock3 size={15}/>}</i><span><b>{type}</b><small>{status==="approved"?`تم التأكيد${row?.decided_by_name?` بواسطة ${row.decided_by_name}`:""}`:status==="rejected"?"يحتاج مراجعة":"لم يُحسم بعد"}</small></span>{canApprove&&<div><button disabled={saving} onClick={()=>void execute({action:"record_approval",accountId:selected.account_id,opportunityId:selected.opportunity_id,partnershipId:selected.partnership_id||"",approvalType:type,decision:"approved"})}>تأكيد</button><button disabled={saving} onClick={()=>void execute({action:"record_approval",accountId:selected.account_id,opportunityId:selected.opportunity_id,partnershipId:selected.partnership_id||"",approvalType:type,decision:"rejected"})}>إرجاع</button></div>}</article>})}</div></section>
}
