...
function B2BWorkspace({section,canManage,canConvert}:{section:"business"|"partnerships";canManage:boolean;canConvert:boolean}) {
  ...
  return <div className="b2b-workspace">
    <section className="b2b-hero"><div><span>{scope==="all"?"نظرة شاملة لجميع الفرق":"مساحة العمل المسندة إليك"}</span><h2>{section==="business"?"مبيعات الأعمال":"الشراكات"}</h2><p>{section==="business"?"كل جهة، تواصل، اجتماع وعرض في مسار واحد واضح.":"متابعة الاتفاقيات والتفعيل والتجديد دون خلطها بمالية الأفراد."}</p></div>{((section==="business"&&canManage)||(section==="partnerships"&&canCreatePartnership))&&<button onClick={()=>{if(section==="partnerships")setForm(current=>({...current,name:"",region:"",city:"",email:"",source:"",path:"",partnershipType:"",priority:"",ownerEmail:"",contactStatus:""}));setShowForm(true)}}><UserPlus size={17}/> إضافة جهة</button>}</section>
    ...
    {showForm&&<section className="b2b-form-card"><header><div><b>{section==="business"?"إضافة جهة لمبيعات الأعمال":"إضافة جهة إلى الشراكات"}</b><span>{section==="partnerships"?"المرحلة الأولى · البيانات الأولية":formStep===1?"الخطوة 1 من 2 · بيانات الفرصة الأساسية":"الخطوة 2 من 2 · تفاصيل العقد والاعتماد"}</span></div><button aria-label="إغلاق النموذج" onClick={()=>{setShowForm(false);setFormStep(1)}}>×</button></header>
      ...
      {formStep===1?(section==="partnerships"?<div className="b2b-form-grid partnership-initial-fields">
        ...
        <label><span>المصدر</span><select value={form.source} onChange={e=>setForm({...form,source:e.target.value})}><option value="">غير محدد</option>{["عميل سابق","قاعدة بيانات سلوكيرا","هو تواصل معنا"].map(x=><option key={x}>{x}</option>)}</select></label>
        <label><span>المجال</span><select value={form.path} onChange={e=>setForm({...form,path:e.target.value})}><option value="">غير محدد</option><option>ABA</option><option>OBM</option><option>BOTH</option></select></label>
