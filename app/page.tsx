...
function B2BWorkspace({section,canManage,canConvert}:{section:"business"|"partnerships";canManage:boolean;canConvert:boolean}) {
  const [rows,setRows]=useState<B2BRow[]>([]),[options,setOptions]=useState<string[]>([]),[loading,setLoading]=useState(true),[error,setError]=useState(""),[showForm,setShowForm]=useState(false),[selected,setSelected]=useState<B2BRow|null>(null),[saving,setSaving]=useState(false),[activities,setActivities]=useState<B2BRow[]>([]),[activityType,setActivityType]=useState("تم التواصل"),[activityDetails,setActivityDetails]=useState(""),[scope,setScope]=useState("assigned"),[canReview,setCanReview]=useState(false),[canCreatePartnership,setCanCreatePartnership]=useState(false),[search,setSearch]=useState(""),[stageFilter,setStageFilter]=useState("الكل"),[pathFilter,setPathFilter]=useState("الكل");
  const [form,setForm]=useState<Record<string,string>>({name:"",type:"مركز",contactName:"",jobTitle:"",phone:"",email:"",region:"",city:"",activity:"",source:"",priority:"متوسطة",path:"ABA",ownerEmail:"",expectedValue:"",expectedCloseDate:"",nextFollowUp:"",recommendedServices:""});
  ...
  const load=async()=>{setLoading(true);setError("");try{const data=await apiJson(`/api/b2b?section=${section}`);setRows(data.opportunities||data.partnerships||[]);setOptions(data.stages||data.statuses||[]);setScope(data.scope||"assigned");setCanReview(Boolean(data.canReview));setCanCreatePartnership(Boolean(data.canCreatePartnership))}catch(e){setError((e as Error).message)}finally{setLoading(false)}};
  ...
  const contacted=rows.filter(row=>!["جهة مسندة",""].includes(String(row.stage||""))).length,contactedToday=rows.filter(row=>String(row.last_contact_at||"").slice(0,10)===today).length,awaitingSignature=rows.filter(row=>row.stage==="بانتظار التوقيع").length;
  const visibleRows=rows.filter(row=>{const q=search.trim().toLowerCase();return(!q||[row.account_name,row.contact_name,row.contact_phone,row.city].some(value=>String(value||"").toLowerCase().includes(q)))&&(stageFilter==="الكل"||String(row.stage||row.status)===stageFilter)&&(pathFilter==="الكل"||String(row.path)===pathFilter)});
  ...
  return <div className="b2b-workspace">
    ...
    <div className="b2b-kpis">
      ...
      <article><span>{section==="business"?"قيمة الفرص":"قيمة الاتفاقيات"}</span><b>{totalValue.toLocaleString("en-US")} <small>ر.س</small></b><small>مؤشر B2B مستقل</small></article>
    </div>
    {section==="business"&&<section className="b2b-funnel"><header><div><b>تقدم الجهات</b><span>قراءة سريعة لمسار التحويل ومكان توقف المتابعة</span></div></header><div>{funnelStages.map((stage,index)=>{const count=rows.filter(row=>funnelStages.indexOf(String(row.stage))>=index).length,percent=rows.length?Math.round(count/rows.length*100):0;return <button key={stage} onClick={()=>setStageFilter(stage)}><span>{stage}</span><b>{count}</b><i><em style={{width:`${percent}%`}}/></i></button>})}</div></section>}
    {error&&<p className="error">{error}</p>}
    {showForm&&<section className="b2b-form-card"><header><div><b>{section==="business"?"إضافة جهة لمبيعات الأعمال":"إضافة جهة إلى الشراكات"}</b><span>{section==="business"?"طلب الموظف ينتقل للاعتماد قبل بدء مسار العمل":"متاحة فقط للإدارة ومن تمنحه صلاحية إضافة الشراكات"}</span></div><button onClick={()=>setShowForm(false)}>×</button></header><div className="b2b-form-grid">
      ...
      <label><span>مسار الجهة</span><select value={form.path} onChange={e=>setForm({...form,path:e.target.value})}><option>ABA</option><option>OBM</option></select></label><label><span>إسناد الموظف (البريد)</span><input type="email" placeholder="يُترك فارغًا لإسنادها لك" value={form.ownerEmail} onChange={e=>setForm({...form,ownerEmail:e.target.value})}/></label>
      {section==="partnerships"&&<>{[['signedAt','تاريخ التوقيع'],['startDate','بداية الاتفاقية'],['endDate','نهاية الاتفاقية'],['agreementNumber','رقم الاتفاقية'],['value','قيمة الاتفاقية']].map(([key,label])=><label key={key}><span>{label} {key.includes('Date')||key==='signedAt'?'*':''}</span><input type={key.includes('Date')||key==='signedAt'?'date':key==='value'?'number':'text'} value={agreement[key]||""} onChange={e=>setAgreement({...agreement,[key]:e.target.value})}/></label>)}</>}
    </div><footer><button disabled={saving} onClick={()=>void save(section==="business"?{action:"create_business",...form}:{action:"create_partnership",...form,...agreement})}>{section==="business"?"إرسال الجهة للاعتماد":"إضافة الجهة إلى الشراكات"}</button></footer></section>}
    <section className="b2b-list"><header><div><b>{section==="business"?"قائمة الجهات":"سجل الشراكات"}</b><span>{visibleRows.length} من {rows.length} جهة</span></div></header>
      <div className="b2b-toolbar"><label><Search size={16}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="ابحث باسم الجهة أو المسؤول أو الجوال..."/></label><select value={pathFilter} onChange={e=>setPathFilter(e.target.value)}><option>الكل</option><option>ABA</option><option>OBM</option></select><select value={stageFilter} onChange={e=>setStageFilter(e.target.value)}><option>الكل</option>{options.map(x=><option key={x}>{x}</option>)}</select>{(search||stageFilter!=="الكل"||pathFilter!=="الكل")&&<button onClick={()=>{setSearch("");setStageFilter("الكل");setPathFilter("الكل")}}>مسح التصفية</button>}</div>
      {loading?<LiveState loading error="" empty={false}/>:visibleRows.length===0?<LiveState loading={false} error="" empty/>:visibleRows.map(row=>{const remaining=days(row.end_date);return <article className="b2b-row" key={String(row.id)} onClick={()=>openRow(row)}>
        ...
        {section==="business"?<><div><span>المسار</span><b>{row.path||"ABA"}</b></div><div><span>{row.approval_status==="pending"?"حالة الاعتماد":"المرحلة"}</span>{row.approval_status==="pending"?<em className="b2b-status orange">بانتظار الاعتماد</em>:canManage?<select className="b2b-inline-stage" value={String(row.stage)} onClick={e=>e.stopPropagation()} onChange={e=>{e.stopPropagation();void save({action:"update_stage",opportunityId:row.id,stage:e.target.value})}}>{options.map(x=><option key={x}>{x}</option>)}</select>:<em className={`b2b-status ${b2bStageColors[String(row.stage)]||"gray"}`}>{row.stage}</em>}</div><div>{row.approval_status==="pending"&&canReview?<div className="b2b-review-actions"><button onClick={e=>{e.stopPropagation();void save({action:"review_business",opportunityId:row.id,decision:"approved"})}}>اعتماد</button><button onClick={e=>{e.stopPropagation();void save({action:"review_business",opportunityId:row.id,decision:"rejected"})}}>رفض</button></div>:<><span>المتابعة القادمة</span><b>{row.next_follow_up||"غير محددة"}</b></>}</div></>:<><div><span>المسار</span><b>{row.path||"ABA"}</b></div><div><span>حالة الشراكة</span><em className={`b2b-status ${row.status==="نشطة"?"green":remaining!==null&&remaining<=60?"orange":"blue"}`}>{row.status}</em></div><div><span>مدة الاتفاقية</span><b>{row.start_date} — {row.end_date}</b><small>{remaining===null?"":remaining<0?"منتهية":`${remaining} يوم متبقٍ`}</small></div></>}
