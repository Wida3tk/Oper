...
const B2B_LIFECYCLE = ["الاستكشاف والتقييم","التفاوض والهيكلة","التفعيل والعمليات","قياس الأثر","التجديد أو الخروج"];
...
function B2BWorkspace({section,canManage,canConvert}:{section:"business"|"partnerships";canManage:boolean;canConvert:boolean}) {
  ...
  const [form,setForm]=useState<Record<string,string>>({name:"",type:"مركز",contactName:"",jobTitle:"",phone:"",email:"",region:"",city:"",activity:"",source:"",priority:"متوسطة",path:"ABA",ownerEmail:"",opportunityKind:"partnership",registrationStatus:"فرصة جديدة",traineeCount:"",requestedProgram:"",deliveryDate:"",expectedValue:"",expectedCloseDate:"",nextFollowUp:"",recommendedServices:""});
  ...
  const loadActivities=async(accountId:unknown)=>{try{const data=await apiJson(`/api/b2b?accountId=${encodeURIComponent(String(accountId||""))}`);setActivities(data.activities||[]);setDocuments(data.documents||[]);setApprovals(data.approvals||[])}catch(e){setError((e as Error).message)}};
  ...
  return <div className="b2b-workspace">
    <section className="b2b-hero"><div><span>{scope==="all"?"نظرة شاملة لجميع الفرق":"مساحة العمل المسندة إليك"}</span><h2>{section==="business"?"مبيعات الأعمال":"الشراكات"}</h2><p>{section==="business"?"كل جهة، تواصل، اجتماع وعرض في مسار واحد واضح.":"متابعة الاتفاقيات والتفعيل والتجديد دون خلطها بمالية الأفراد."}</p></div>{((section==="business"&&canManage)||(section==="partnerships"&&canCreatePartnership))&&<button onClick={()=>setShowForm(true)}><UserPlus size={17}/> إضافة جهة</button>}</section>
    ...
    {showForm&&<section className="b2b-form-card"><header><div><b>{section==="business"?"إضافة جهة لمبيعات الأعمال":"إضافة جهة إلى الشراكات"}</b><span>{formStep===1?"الخطوة 1 من 2 · بيانات الفرصة الأساسية":"الخطوة 2 من 2 · تفاصيل العقد والاعتماد"}</span></div><button aria-label="إغلاق النموذج" onClick={()=>{setShowForm(false);setFormStep(1)}}>×</button></header>
      <div className="b2b-form-shell">
        <aside className="b2b-form-sidebar" aria-label="مراحل إضافة الجهة">
          ...
          <button type="button" className={formStep===1?"current":"complete"} onClick={()=>setFormStep(1)}><i>{formStep>1?<Check size={15}/>:"01"}</i><span><b>بيانات الجهة</b><small>الجهة، مسؤول التواصل، المسار والإسناد</small></span></button>
          <button type="button" disabled={section==="business"&&form.registrationStatus!=="موقعة"} className={formStep===2?"current":""} onClick={()=>{if(section==="partnerships"||form.registrationStatus==="موقعة")setFormStep(2)}}><i>02</i><span><b>الاتفاقية والاعتماد</b><small>{section==="business"&&form.registrationStatus!=="موقعة"?"تُفتح عند اختيار موقعة":"التواريخ والقيمة وملف الاتفاقية"}</small></span></button>
        </aside>
        <div className="b2b-form-main">
          <div className="b2b-form-section-head"><i>{formStep===1?"01":"02"}</i><div><h3>{formStep===1?"بيانات الجهة والفرصة":"تفاصيل الاتفاقية"}</h3><p>{formStep===1?"سجّل البيانات الأساسية وحدد المسؤول والمسار التشغيلي.":"أكمل بيانات التوقيع والاتفاقية قبل إنشاء الشراكة."}</p></div></div>
      {formStep===1?<div className="b2b-form-grid">
      ...
      <footer>{formStep===2&&<button className="secondary" onClick={()=>setFormStep(1)}>السابق</button>}<button disabled={saving||!form.name||!form.contactName} onClick={()=>{const needsContract=section==="partnerships"||form.registrationStatus==="موقعة";if(formStep===1&&needsContract){setFormStep(2);return}void save(formStep===2?{action:"create_partnership",...form,...agreement}:{action:"create_business",...form})}}>{formStep===1&&(section==="business"&&form.registrationStatus!=="موقعة")?"إرسال الجهة للاعتماد":formStep===1?"التالي":"حفظ الشراكة"}</button></footer>
    </section>}
...
function B2BApprovalsPanel({selected,approvals,canApprove,saving,execute}:{selected:B2BRow;approvals:B2BRow[];canApprove:boolean;saving:boolean;execute:(body:Record<string,unknown>)=>Promise<void>}){
  const types=["مدير الشراكات","الإدارة القانونية","الإدارة المالية","الاعتماد النهائي"];
  const latest=(type:string)=>approvals.find(x=>x.approval_type===type);
  return <section className="b2b-approvals"><div className="b2b-progress-head"><div><h3>بوابات الموافقة</h3><p>قرارات موثقة قبل الانتقال بين المراحل</p></div></div><div>{types.map(type=>{const row=latest(type),status=String(row?.status||"pending");return <article key={type}><i className={status}>{status==="approved"?<Check size={15}/>:status==="rejected"?<X size={15}/>:<Clock3 size={15}/>}</i><span><b>{type}</b><small>{status==="approved"?`معتمد بواسطة ${row?.decided_by_email}`:status==="rejected"?"مرفوض ويحتاج معالجة":"بانتظار القرار"}</small></span>{canApprove&&<div><button disabled={saving} onClick={()=>void execute({action:"record_approval",accountId:selected.account_id,opportunityId:selected.opportunity_id,partnershipId:selected.partnership_id||"",approvalType:type,decision:"approved"})}>اعتماد</button><button disabled={saving} onClick={()=>void execute({action:"record_approval",accountId:selected.account_id,opportunityId:selected.opportunity_id,partnershipId:selected.partnership_id||"",approvalType:type,decision:"rejected"})}>رفض</button></div>}</article>})}</div></section>
}
