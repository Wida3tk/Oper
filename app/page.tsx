...
function B2BWorkspace({section,canManage,canConvert}:{section:"business"|"partnerships";canManage:boolean;canConvert:boolean}) {
  const [rows,setRows]=useState<B2BRow[]>([]),[options,setOptions]=useState<string[]>([]),[partnershipStages,setPartnershipStages]=useState<string[]>([]),[trainingStages,setTrainingStages]=useState<string[]>([]),[loading,setLoading]=useState(true),[error,setError]=useState(""),[showForm,setShowForm]=useState(false),[selected,setSelected]=useState<B2BRow|null>(null),[saving,setSaving]=useState(false),[activities,setActivities]=useState<B2BRow[]>([]),[activityType,setActivityType]=useState("تم التواصل"),[activityDetails,setActivityDetails]=useState(""),[scope,setScope]=useState("assigned"),[canReview,setCanReview]=useState(false),[canCreatePartnership,setCanCreatePartnership]=useState(false),[canDelete,setCanDelete]=useState(false),[search,setSearch]=useState(""),[stageFilter,setStageFilter]=useState("الكل"),[pathFilter,setPathFilter]=useState("الكل"),[kindFilter,setKindFilter]=useState("الكل");
  const [form,setForm]=useState<Record<string,string>>({name:"",type:"مركز",contactName:"",jobTitle:"",phone:"",email:"",region:"",city:"",activity:"",source:"",priority:"متوسطة",path:"ABA",ownerEmail:"",opportunityKind:"partnership",traineeCount:"",requestedProgram:"",deliveryDate:"",expectedValue:"",expectedCloseDate:"",nextFollowUp:"",recommendedServices:""});
  ...
  return <div className="b2b-workspace">
    ...
    {showForm&&<section className="b2b-form-card"><header><div><b>{section==="business"?"إضافة جهة لمبيعات الأعمال":"إضافة جهة إلى الشراكات"}</b><span>{section==="business"?"طلب الموظف ينتقل للاعتماد قبل بدء مسار العمل":"متاحة فقط للإدارة ومن تمنحه صلاحية إضافة الشراكات"}</span></div><button onClick={()=>setShowForm(false)}>×</button></header><div className="b2b-form-grid">
      ...
      {section==="business"&&<label><span>نوع الفرصة</span><select value={form.opportunityKind} onChange={e=>setForm({...form,opportunityKind:e.target.value})}><option value="partnership">فرصة شراكة</option><option value="corporate_training">طلب تدريب مؤسسي</option></select></label>}
